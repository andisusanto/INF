...
Public Class Customer
    ...
    <Action("FixBugs")>
    Public Sub FixBugs()
        Dim xp As New XPCollection(Of StatementAmount)(Session, New BinaryOperator("OutstandingAmount", 0, BinaryOperatorType.Greater))
        Dim cnt As Integer = 1
        For Each obj In xp
            Dim objPayment As New SalesPayment(Session) With {.Customer = obj.Statement.Customer, .TransDate = Today, .Period = obj.Statement.Period, .Currency = obj.Currency, .Amount = obj.Amount}
            objPayment.Code = String.Format("{0:00000}", cnt)
            cnt += 1
            objPayment.Submit()
        Next
    End Sub
